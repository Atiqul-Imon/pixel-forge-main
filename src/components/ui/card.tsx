...
const Card = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(
        'rounded-2xl border border-slate-200/80 bg-card text-card-foreground shadow-float backdrop-blur-sm transition-all duration-300 hover:border-slate-200 hover:shadow-elevated-sm',
        className
      )}
      {...props}
    />
